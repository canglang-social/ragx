...
export async function answerQuestion(question: string, deps: RagDeps): Promise<RagResult> {
  const { embedder, store, generator, reranker = new IdentityReranker(), topK = 5 } = deps;
