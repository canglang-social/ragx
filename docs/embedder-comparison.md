...
| Embedder (model) | dim | Retrieval hit@20 | Answer | Notes |
| --- | --- | --- | --- | --- |
| Jina `jina-embeddings-v3` | 1024 | **0.94** (16/17) | **0.95** (19/20) | free tier; current shipped baseline. Only miss: q008 (equity sentence ranks 77 — competed). |
| Jina `jina-embeddings-v5-text-small` | ? | _pending_ | _pending_ | next test |

Method note: the only failure on the baseline (q008) is a *ranking* miss — the gold
chunk is recalled but ranks too low. A stronger embedder may or may not lift it; the
table records what each actually does.
