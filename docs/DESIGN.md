...
| v2 | Agentic retrieval. **Trigger fired:** cross-document comparison was 0/6 — a single query vector can't reach two filings. Fix: **query decomposition** (`Planner` seam → per-entity sub-queries → merge) + **hybrid BM25+vector retrieval** (RRF) to surface buried figures. One branch, not a loop — no LangGraph yet (waits for true cycles). **Measured: retrieval 0.63→0.97, answer 0.78→0.98.** Built in signal-driven layers: decomposition grounds cross-doc (was 0/6); hybrid surfaces buried figures (a wash alone — unlocked by decomposition); RRF weights vector 1.2× (tuned on a rank probe); **contextual retrieval + a cross-encoder reranker** (0.72→0.85) — a `company — year — section` header on the embedding/BM25 text fixes table-row recall, a reranker over a wide set fixes rank, and the reranker reads the RAW chunk not the header (separation matters; replacing was a wash); then **LLM table-row descriptions** ("Family 3", 0.85→0.97) crack the statement-row frontier — a bare row no embedder ranks is rewritten by one LLM call/page into a sentence embedded in its place (the raw row kept for grounding), and cross-doc queries rerank each entity against *its own* sub-query so a comparison keeps both figures. | validated on eval; deployable on a fully-hosted, vendor-swappable stack (SiliconFlow Qwen3-Embedding-8B + Qwen3-Reranker-8B + DeepSeek — embedder/reranker are env-switched, also run Jina or local Ollama). |
