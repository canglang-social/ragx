...
- **Multi-hop reasoning — a small probe is encouraging, not yet a verdict.** A 4-case probe (q021–q024) showed the linear pipeline + a capable generator (Groq 70b) computing same-chunk *and* one cross-page difference correctly (2023 net earnings − operating earnings, facts on pages 82 and 6). That *suggests* the linear pipeline may handle multi-hop without an agent — but **4 cases (one cross-page) is far too small to decide.** Properly answering "does v2 earn its place?" needs a larger eval with many harder multi-hop + cross-document questions; until then v2 is *open*, not declined. So far the multi-hop *failures* were retrieval misses, not reasoning; the other hard case (q013) is dense-table retrieval.
